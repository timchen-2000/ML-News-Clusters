...
cluster_labels = dbscan.fit_predict(reduced_tfidf_matrix_100)
# 将聚类结果添加到数据框中
reduced_tfidf_matrix['cluster'] = cluster_labels
# 打印每个簇的数量
...
# 打印不同簇的数量
num_clusters = len(unique) - (1 if -1 in cluster_labels else 0)
print(f"不同簇的数量（不包括噪声点）: {num_clusters}")
print(f"噪声点的数量: {cluster_counts.get(-1, 0)}")

# 打印 cluster_labels 中的唯一簇的数量
print(f"cluster_labels 中的唯一簇的数量（包括噪声点）: {len(unique)}")
...
# 合并聚类结果和文章基本信息
clustered_data_with_info = pd.concat([reduced_tfidf_matrix, basic_info_df[['publication', 'author', 'year', 'month']]], axis=1)

# 保存带有聚类结果和基本信息的数据
clustered_data_with_info_path = r'D:\machine_learning_class_project\datasets\Clustered_dbscan_test_.csv'
...
# 查看聚类结果分布
print(clustered_data_with_info['cluster'].value_counts())
...
colors = plt.cm.Paired(np.linspace(0, 1, num_clusters))
